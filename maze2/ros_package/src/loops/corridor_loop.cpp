...
void CorridorLoop::state_drive_forward()
{
    ...
    else if (crossroad_detected())
    {
        ...
        if (crossroad_ticks_ >= CROSSROAD_CONFIRM_TICKS)
        {
            ...
            crossroad_ticks_   = 0;
            pid_heading_.reset();
            crossroad_inhibit_ticks_   = CROSSROAD_COOLDOWN_TICKS;
            pid_centering_.reset();
            lidar_mode_active_ = false;
            state_             = State::STOP_AND_SCAN;
            RCLCPP_WARN(this->get_logger(),
...
void CorridorLoop::state_stop_and_scan()
{
    ...
    const bool south_open = south_ > OPEN_THRESHOLD || south_ < VALID_MIN;

    // Count only N/E/W — south is never counted.
    const int open_count = (north_open ? 1 : 0) + (east_open ? 1 : 0) + (west_open ? 1 : 0) + (south_open ? 1 : 0);
...
bool CorridorLoop::crossroad_detected() const
{
    ...
    const bool s = south_ > OPEN_THRESHOLD || south_ < VALID_MIN;
    return (static_cast<int>(n) + static_cast<int>(e) + static_cast<int>(w) + static_cast<int>(s)) > 2;
}
